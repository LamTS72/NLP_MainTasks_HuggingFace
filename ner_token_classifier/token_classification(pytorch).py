...
class TokenClassifier():
    ...
    def get_pair_items(self, set="train", index=0, feature1="tokens", feature2="ner_tags"):
        ...
        line1 = ""
        line2 = ""
        for w1, w2 in zip(feature1, feature2):
            #featureName = self.get_featureName_items(set, feature2)
            full_label = self.labelName[w2]
            max_length = max(len(w1), len(full_label))
            line1 += str(w1) + str(" " * (max_length - len(w1) + 1))
    ...
    def get_tokenizer(self, set="train",index=0, feature="tokens", is_split_into_words=True):
        ...
        inputs = self.tokenizer(self.raw_datasets[set][index][feature], is_split_into_words=is_split_into_words)
    ...
    def create_argumentTrainer(self, output_dir="fine-tuned-model", eval_strategy="epoch", 
                               save_strategy="epoch", learning_rate=2e-5,num_train_epochs=1, weight_decay=0.01, push_to_hub=False, 
                               hub_model_id=""):
        # num_update_steps_per_epoch = len(self.tokenized_dataset["train"])
    ...
    def call_train(self, model_path="pretrained-model",set_train="train", set_val="validation", push_to_hub=False):
        trainer = Trainer(
            model=self.model,
            args=self.args,
    ...
    def call_pipeline(self, local=False, path="fine-tuned-model/checkpoint-1756", example=""):
        if local:
            model_checkpoint = "Chessmen/token_classify"
        else:
...
if __name__ == "__main__":
    ...
    '''
        2_EXPLORING DATASET
    '''
    print("Example 0 in dataset: ",ner.get_feature_items(set="train", index=0, feature="tokens"))
    '''
    ...
    print("Number of labels in NER: ",ner.model.config.num_labels)
